...
def get_delayed_vector(time_vals, master_array, current_time, i_position, j, last_frame):
    for frame_delay in range(last_frame + 1):
        ...
        j_position = master_array[last_frame - frame_delay][j, 0:3]
        rij = i_position - j_position
        dist_ij = (rij[0]**2 + rij[1]**2 + rij[2]**2)**(1/2)

        #Sign flip indicating that the delay value has been reached
        if time_delay >= dist_ij / g_speed:
            return rij, dist_ij**(-3)

    #Mass i is not yet affected by the pull of mass j
    return rij, 0
...
def calcAcc(time_vals, master_array, current_time, position_and_velocity, last_frame, mass):
    ...
    for i in range(n_bodies):
        ...
        for j in range(n_bodies):
            ...
            rij, inv_r3 = get_delayed_vector(time_vals, master_array, current_time, i_position, j, last_frame)
...
def int_rk4(f, u_init, h, frames, masses):
    ...
    for i in range(frames - 1):
        ...
        k1 = f(time_vals, master_array, current_time, position_and_velocity, i, masses)
        k2 = f(time_vals, master_array, current_time + h/2, position_and_velocity + k1 * h/2, i, masses)
        k3 = f(time_vals, master_array, current_time + h/2, position_and_velocity + k2 * h/2, i, masses)
        k4 = f(time_vals, master_array, current_time + h, position_and_velocity + k3 * h, i, masses)
...
def int_rkf(f, t0, y0, t_bound, tol, masses):
    ...
    while tk < t_bound:
        ...
        # update y
        k1 = h * f(x_array, y_array, tk, yk, fcount, masses)
        k2 = h * f(x_array, y_array, tk + c2 * h, yk + c2 * k1, fcount, masses)
        k3 = h * f(x_array, y_array, tk + c3 * h, yk + c32 * k1 + c33 * k2, fcount, masses)
        k4 = h * f(x_array, y_array, tk + c4 * h, yk + c42 * k1 + c43 * k2 + c44 * k3, fcount, masses)
        k5 = h * f(x_array, y_array, tk + h, yk + c5 * k1 - c52 * k2 + c53 * k3 + c54 * k4, fcount, masses)
        k6 = h * f(x_array, y_array, tk + c6 * h, yk + c62 * k1 + c63 * k2 + c64 * k3 + c65 * k4 + c66 * k5, fcount, masses)
...
def int_yos(f, u_init, v0, h, frames, masses):
    ...
    for i in range(frames - 1):
        ...
        u1 = u0 + h * c1 * v0
        v1 = v0 + h * w1 * f(time_vals, master_array, current_time, u1, i, masses)
        u2 = u1 + h * c2 * v1
        v2 = v1 + h * w0 * f(time_vals, master_array, current_time, u2, i, masses)
        u3 = u2 + h * c2 * v2
        v3 = v2 + h * w1 * f(time_vals, master_array, current_time, u3, i, masses)
        u4 = u3 + h * c1 * v3
        v0 = v3
...
# run simulation
#(current_time, position_array) = int_yos(calcAcc, start[:,0:3], start[:,3:], h, frames, masses)
(current_time, position_array) = int_rk4(calcAcc, start, h, frames, masses)
#(current_time, position_array) = int_rkf(calcAcc, 0, start, 50, 1e-5, masses)
#frames = len(current_time)

print(position_array[-1, -1, :])
...
if print_exit:
    ...
    for i in range(column_len):
        ax.plot(position_array[:, i, 0], position_array[:, i, 1], position_array[:, i, 2])
...
for i in range(num_particles):
    particle_postion = position_array[:,i,0:3]
...
#max_x = position_array[-1,-1,0] + 1
#ax.set_xlim(-1, max_x - 1)
#ax.set_ylim(-max_x/2, max_x/2)
#ax.set_zlim(-max_x/2, max_x/2)

# animation function
def update(frames, points, tails):
    current_positions = position_array[frames, :, 0:3]
...
ani = FuncAnimation(fig, update, frames=range(0,frames,4), fargs=(points, tails), interval=10)
plt.show()
