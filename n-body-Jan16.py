...
masses = np.array([250, 123])#, 200, 500])

def find_tau(frame_number, tvals, uvals, i_position, j):

    for tau in range(0, frame_number):

        time_delay = tvals[frame_number] - tvals[frame_number - tau]

        j_position = uvals[frame_number - tau, j, 0:3]

        #[distance between i_position and j_position] minus [time_delay * c] to flip signs
        if 1: #change this
            return 1

    return 0

def calcAcc(frame_number, tvals, uvals, t, u, mass):
    mass = masses
    ...
    for i in range(n_bodies):
        ...
        for j in range(n_bodies):
            ...
            i_position = u[i, 0:3]
            tau = find_tau(frame_number, tvals, uvals, i_position, j)
...
def int_rk4(f, u_init, h, num, masses):
    ...
    for i in range(num - 1):
        ...
        k1 = f(i, tvals, master_array, t, u, masses)
        k2 = f(i, tvals, master_array, t + h/2, u + k1 * h/2, masses)
        k3 = f(i, tvals, master_array, t + h/2, u + k2 * h/2, masses)
        k4 = f(i, tvals, master_array, t + h, u + k3 * h, masses)
